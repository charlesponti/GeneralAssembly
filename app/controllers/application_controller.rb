class ApplicationController < ActionController::Base
  ...
  def current_user
    # @current_user ||=  case session[:role]
    #   when 'student' then Student.find(session[:user_id]) if session[:user_id]
    #   when 'teacher' then Teacher.find(session[:user_id]) if session[:user_id]
    # end
